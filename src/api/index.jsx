  ...
  try {
    const [gameData, characters] = await Promise.all([
      ...
      db
        ...
        .leftJoin(
          schema.characters,
          eq(schema.foundCharacters.characterId, schema.characters.id),
        )
...
app.post("/games/:id", async (c) => {
  ...
  const [character, found, startTime] = await Promise.all([
    db
      ...
      .where(
        and(
          gte(x, schema.characters.xMin),
          lte(x, schema.characters.xMax),
          gte(y, schema.characters.yMin),
          lte(y, schema.characters.yMax),
        ),
      ),
    db
      .select({
  ...
  if (found.some((foundChar) => foundChar.id === foundCharacter.id)) {
    return c.newResponse(
      JSON.stringify({ error: "You've already found that character" }),
      400,
    )
  }
  if (found.length === 3) {
      ...
      const [found, score] = await db.batch([
        ...
        db
          ...
          .set({
            ...
            scoreInSeconds: sql`(CAST(strftime('%s', current_timestamp) AS INTEGER) - CAST(strftime('%s', ${startTime[0].startTime}) AS INTEGER))`,
          })
          .where(eq(schema.games.id, id))
...
app.patch("/games/:id", async (c) => {
  ...
  const response = await db
    .update(schema.games)
    .set({ playerName: name })
    .where(and(eq(schema.games.id, id), isNull(schema.games.playerName)))
    .returning({ playerName: schema.games.playerName })
...
app.get("/leaderboard", async (c) => {
  ...
  const response = await db
    ...
    .where(
      and(
        isNotNull(schema.games.scoreInSeconds),
        isNotNull(schema.games.playerName),
      ),
    )
