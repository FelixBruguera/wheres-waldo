...
let mockPatchResponse = (name) => {
  return { ok: true, json: () => Promise.resolve({ playerName: name }) };
};
...
global.fetch = vi.fn((url, options) => {
  ...
  return Promise.reject(
    new Error(`Unhandled fetch request to ${url} with ${options}`),
  );
});
...
describe("Game component", () => {
  it("renders correctly with a new game", async () => {
    ...
    expect(await screen.findByLabelText("Time Elapsed")).toHaveTextContent(
      "00:00:00",
    );
    expect(await screen.findByLabelText("Characters found")).toHaveTextContent(
      "0 / 4",
    );
  ...
  it("shows a notification and a marker with the correct styling after a good guess", async () => {
    ...
    mockPostResponse = {
      ...
      status: 200,
      json: () =>
        Promise.resolve({ status: "playing", found: { name: "Waldo", id: 1 } }),
    };
    renderWithRouter(<Game />, "/123", "/:id");
    await user.click(await screen.findByRole("img"));
  ...
  it("shows the correct error message after guessing an already found character", async () => {
    ...
    mockPostResponse = {
      ...
      status: 400,
      json: () =>
        Promise.resolve({ error: "You've already found that character" }),
    };
    await user.click(await screen.findByRole("img"));
    expect(
      await screen.getByText("You've already found that character"),
    ).toBeVisible();
    expect(await screen.findByLabelText("Characters found")).toHaveTextContent(
      "1 / 4",
    );
  ...
  it("updates the name after submiting the GameOver form", async () => {
    ...
    expect(await screen.getByPlaceholderText("Claim your time")).toHaveValue(
      "Testing",
    );
  ...
  it("shows the correct error message when the patch request fails", async () => {
    ...
    mockPatchResponse = (name) => {
      return {
        ok: false,
        json: () =>
          Promise.resolve({
            error: "Name must have more than 3 and less than 20 characters",
          }),
      };
    };
    renderWithRouter(<Game />, "/123", "/:id");
    ...
    expect(
      await screen.findByText(
        "Name must have more than 3 and less than 20 characters",
      ),
    ).toBeVisible();
    expect(
      await screen.queryByPlaceholderText("Claim your time"),
    ).not.toBeNull();
