...
const Game = () => {
  ...
  const checkSelection = async (clientCoords, normalizedCoords) => {
    ...
    const response = await fetch(`/api/games/${id}`, {
      method: "POST",
      body: JSON.stringify({ x: normalizedCoords[0], y: normalizedCoords[1] }),
    })
    ...
    if (response.ok) {
      ...
      if (data.status === "finished") {
        ...
        setNotification({
          message: `You've found ${data.found.name}!`,
    ...
    <div className="bg-background w-dvw h-dvh">
      ...
      <nav className="fixed bg-background w-dvw h-12 flex items-center justify-between px-2">
        ...
        <div className="flex items-center justify-evenly w-6/10 lg:w-4/10">
          ...
          <p aria-label="Characters found" title="Characters found">
            <span className="text-accent font-bold">{found.length}</span> / 4
          </p>
        </div>
      </nav>
      <ImageContainer
        markers={markers}
        onClick={({ client, normalized }) => checkSelection(client, normalized)}
      />
    </div>
  )
}
