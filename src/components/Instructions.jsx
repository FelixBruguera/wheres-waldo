...
const Instructions = ({ modal, found }) => {
    ...
    return (
        <div className="flex flex-col w-full items-center gap-6 p-2">
            ...
            <ul className="w-8/10 flex flex-wrap lg:flex-nowrap items-center justify-evenly bg-white mx-auto">
                { Object.entries(characters).map(([key, value]) => {
                    const isFound = found.some(char => char.name === key)
                    return (
                    <li key={key} className="flex flex-col gap-1 lg:h-70">
                        <p className="text-center">{key}</p>
                        <img src={`/${value}`} alt={key} className={`w-30 h-55 lg:size-50 ${isFound && "opacity-50"}`} />
